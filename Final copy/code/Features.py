...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 26 16:39:22 2018

@author: meihuaren & linye
"""
...
class Features():
    '''
    class to create and hold our features
    '''
    def __init__(self, message_filename, orderbook_filename, path=os.getcwd()):
        ...
        #columns names for message and orderbook file
        self._messages.columns = ['time', 'type', 'order ID', 'size', 'price', 'direction']
        self._limit_order_book.columns =  ['{}_{}_{}'.format(s,t,l) for l in range(1,6) for s in ['ask','bid'] for t in ['price', 'vol'] ]
        
        #features
        self._all_features = pd.concat([self._messages.time, self._limit_order_book], axis = 1)
    
    def CreateFeatures(self):
        
        total_data = pd.concat([self._messages, self._limit_order_book], axis = 1)
        total_data['mid_price'] = (total_data.loc[:,'ask_price_1'] + total_data.loc[:,'bid_price_1']) / 2
        total_data['mid_price_mov'] = np.sign(total_data['mid_price'].shift(-1)-total_data['mid_price']) # the last one is nan
        ...
        self._all_features['order_flow_buy'] = total_data['order_flow_buy']
        self._all_features['order_flow_sell'] = total_data['order_flow_sell']
        ...
        '''
        for i in range(1,6):
            total_data['liq_imb_'+str(i)] = total_data['ask_vol_'+str(i)] / (total_data['ask_vol_'+str(i)] + total_data['bid_vol_'+str(i)])
            total_data['liq_imb_'+str(i)].fillna(method='ffill', inplace=True)
            self._all_features['liq_imb_'+str(i)] = total_data['liq_imb_'+str(i)]
        ...
        self._all_features['actual_spread'] = total_data['actual_spread']
        ...
        self._all_features['actual_mkt_imb'] = total_data['actual_mkt_imb']
        ...
        self._all_features['relative_mkt_imb'] = total_data['relative_mkt_imb']
        ...
        self._all_features['relative_mid_price_trend'] = total_data['relative_mid_price_trend']
        ...
        self._all_features['relative_spread'] = total_data['relative_spread']
        ...
        self._all_features['volatility'] = total_data['volatility']
        ...
        self._all_features['lo_agr_ask'] = total_data['lo_agr_ask']
        ...
        self._all_features['lo_agr_bid'] = total_data['lo_agr_bid']
        ...
        self._all_features['effective_spread'] = total_data['effective_spread']
        ...
        self._all_features['ILLIQ'] = total_data['ILLIQ']
        ...
        for i in range(1,6):  
            ...
            self._all_features['rel_bid_vol_'+str(i)] = total_data['rel_bid_vol_'+str(i)]
            self._all_features['rel_ask_vol_'+str(i)] = total_data['rel_ask_vol_'+str(i)]
        ...
        self._all_features['depth_ask_vol'] = total_data['depth_ask_vol']
        self._all_features['depth_bid_vol'] = total_data['depth_bid_vol']
        ...
        for i in range(1,6):  
            total_data['rank_ask_vol_'+str(i)] = total_data['ask_vol_'+str(i)].rolling(50, min_periods = 1).apply(rollrank,raw=True)
            total_data['rank_bid_vol_'+str(i)] = total_data['bid_vol_'+str(i)].rolling(50, min_periods = 1).apply(rollrank,raw=True)
            ...
            self._all_features['rank_bid_vol_'+str(i)] = total_data['rank_bid_vol_'+str(i)]
            self._all_features['rank_ask_vol_'+str(i)] = total_data['rank_ask_vol_'+str(i)]
        ...
        for i in range(1,6):  
            ...
            self._all_features['corr_vol_'+str(i)] = total_data['corr_vol_'+str(i)]
        
        #-- create label
        self._all_features['label'] = total_data['mid_price_mov']
        self._all_features.dropna(inplace = True)
        self._all_features['label'] = self._all_features['label'].astype(int)
        self._all_features.set_index('time', inplace = True)
